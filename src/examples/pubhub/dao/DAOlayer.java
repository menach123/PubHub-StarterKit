...
/**
 * Implementation for the DAOlayer, responsible for querying the database for Book objects.
 */

public class DAOlayer implements BookDAO, BookTagDAO {
...
//				System.out.println(rs.getString("isbn_13"));
	...
	public List<Book> getBooksByTag(String title) {
		...
		try {
			connection = DAOUtilities.getConnection();
			String sql = "SELECT * FROM Books b INNER JOIN Book_Tag bt ON b.isbn_13 = bt.isbn_13 WHERE bt.tag_name LIKE ?";	// Note the ? in the query
			stmt = connection.prepareStatement(sql);
	...
	public BookTag getTagByISBN(String isbn) {
		...
		try {
			...
			if (rs.next()) {
				tag = new BookTag(rs.getString("isbn_13"), rs.getString("tag_name"));			
			}
	...
	public boolean updateBookTag(BookTag tag) {
		try {
			connection = DAOUtilities.getConnection();
			String sql = "UPDATE Book_Tags SET title=?, author=?, price=? WHERE isbn_13=?";
			stmt = connection.prepareStatement(sql);
